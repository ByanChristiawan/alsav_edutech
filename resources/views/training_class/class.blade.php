...
<body>
    ...
    <div id="app" class="hidden">
        ...
        <div class="flex mx-auto max-w-7xl py-3 px-2 sm:px-6 lg:px-8 h-[88vh] w-full gap-5">
            ...
            <div class="hidden md:block h-full pb-5 w-[302px] bg-white rounded-[20px] shadow-custom gap-5">
                <div class="flex justify-between items-center w-full bg-black px-2 py-1 rounded-t-[20px] xl:px-5 xl:py-3">
                  ...
                  <ul id="sidebar" class="py-[15px] w-full h-[85%] text-grays flex-grow overflow-y-scroll">
                    @forelse ($products as $product)
                    <a href="{{ url('/dashboard/class/edu/'.$url.'/'. $product->id ) }}" id="buttonDays" class="flex py-[5px] gap-[6px]">
                      <div class="w-[4px] rounded-r-[20px] {{ ($product->id == $tes) ? 'actives' : ''}}"></div>
                      <li class="hover-color border-b-[1.5px] w-full px-[10px] py-2 hover:rounded-l-lg {{ ($product->id == $tes) ? 'active' : ''}}">{{ $product->hari }}</li>
                    </a>
                    @empty
